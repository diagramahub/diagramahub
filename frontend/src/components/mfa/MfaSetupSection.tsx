      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 mt-6">
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {/* Email MFA card */}
            <div className={`relative rounded-lg border p-4 ${hasEmail ? 'border-green-200 bg-green-50/50' : 'border-gray-200 bg-gray-50/50'}`}>
              <div className="flex items-start gap-3">
                <div className={`flex-shrink-0 p-2 rounded-lg ${hasEmail ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                  <EmailIcon className="w-5 h-5" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h4 className="text-sm font-semibold text-gray-900">{t('mfa.setup.statusEmail')}</h4>
                    {hasEmail && <CheckCircleIcon className="w-4 h-4 text-green-500" />}
                    {status?.default_method === 'email' && (
                      <span className="inline-flex items-center gap-0.5 text-xs bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded-full">
                        <StarIcon className="w-3 h-3" />
                        {t('mfa.setup.defaultMethod')}
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-gray-500 mt-0.5">{t('mfa.setup.emailDescription')}</p>
                  ...
                  {hasEmail ? (
                    ...
                    !showEnableEmail && !showEnableTotp && !disableMethod && (
                      <button
                        ...
                        className="mt-3 px-3 py-1.5 text-xs font-medium text-purple-600 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 transition-colors"
                      >
            ...
            {/* TOTP MFA card */}
            <div className={`relative rounded-lg border p-4 ${hasTotp ? 'border-green-200 bg-green-50/50' : 'border-gray-200 bg-gray-50/50'}`}>
              <div className="flex items-start gap-3">
                <div className={`flex-shrink-0 p-2 rounded-lg ${hasTotp ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                  <ShieldIcon className="w-5 h-5" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h4 className="text-sm font-semibold text-gray-900">{t('mfa.setup.statusTotp')}</h4>
                    {hasTotp && <CheckCircleIcon className="w-4 h-4 text-green-500" />}
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">{t('mfa.setup.totpDescription')}</p>
          ...
          {disableMethod && (
            <form onSubmit={handleDisableMfa} className="bg-red-50 border border-red-200 rounded-lg p-4">
