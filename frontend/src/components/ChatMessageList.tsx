...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      type="button"
      onClick={handleCopy}
      className="mt-1 flex items-center gap-1 text-[10px] text-gray-400 hover:text-gray-600 transition-colors opacity-0 group-hover:opacity-100"
      ...
      {messages.map((msg) => (
        ...
        >
          ...
          {msg.role === 'assistant' && msg.model_used && (
            <span className="text-[10px] text-gray-400 mb-0.5 px-1 flex items-center gap-1">
              {msg.provider_used && (
                <img
              ...
              {msg.generation_time != null && (
                <span className="text-gray-300">• {msg.generation_time.toFixed(1)}s</span>
              )}
            </span>
          )}
          ...
          >
            ...
            {onDeleteMessage && (
              <button
                ...
                className={`absolute -top-2 ${msg.role === 'user' ? '-left-2' : '-right-2'} p-0.5 rounded-full bg-white border border-gray-200 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity shadow-sm`}
            ...
            {msg.role === 'error' && onRetry && (
              <button
                ...
                className="mt-2 px-3 py-1 text-xs font-medium text-red-600 bg-white border border-red-300 rounded-md hover:bg-red-50 transition-colors"
              >
                ↻ Reintentar
              </button>
            )}
